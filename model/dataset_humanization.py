...
def process_dataset(subset, metadata, max_len):
    """Process subset of GMD dataset for Humanization
    ...
    @:return tuple with inputs (only hits with values other than 0), outputs (full-beats) and hvo_sequences (full hvo objects)
    """
    inputs = []
    outputs = []
    hvo_sequences = []
...
class GrooveMidiDatasetHumanization(Dataset):
    """
    Class that loads and processes the GrooveMidiDataset for the task of drum humanization
    ...
    def __init__(self,
        ...
        self.inputs, self.outputs, self.sequences = process_dataset(subset, metadata, max_len)
